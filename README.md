...
**AIが会話文を生成せず、いつも同じ定型文（「私は〜ということばを覚えたいです」）になる場合**

これは、Googleが2026年に導入した新しいAPIキー形式（`AQ.`から始まるキー）が、単純なREST呼び出しでは正しく認証されないことがある、という既知の問題が原因です。`generate-sentence.js` は Google公式のSDK（`@google/genai`）経由で呼び出す実装になっており、この問題を回避しています。もし同じ症状が出た場合は、Netlifyの「Deploys」→「Trigger deploy」→「Clear cache and deploy site」で、依存パッケージを再インストールしてから再デプロイしてみてください。
